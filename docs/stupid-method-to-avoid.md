...
3. **It is one recurring error, not eighteen.** `RM 30,809,399.76` appears twice in this file: as the **wrong** answer in rule 2 and the **right** answer in rule 16. Same dataset, same day — overshot by guessing, undershot by not checking.
4. **Only ~3 of 18 are skill problems** (4, 9, 18); the remaining 15 are belief problems. More Playwright knowledge would have prevented almost none of this.
